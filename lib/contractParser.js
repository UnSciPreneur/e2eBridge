...
function prepareBalanceUpdate(blockNumber, maxBlockNumber) {
  logger.info('*** Processing contracts from blocks ' + blockNumber + ' (0x' + blockNumber.toString(16) + ') onward. ***');
...
function getContractCode(contracts, blockNumber, maxBlockNumber) {
  ...
  for (var i = 0; i < responseCounterTarget; i++) {
    // use a closure to loop over all async calls
    (function (ind, responseCounterTarget, blockNumber, maxBlockNumber, contracts) {
      // take the contract code as it was deployed (and ignore later kill()s)
      var block = '0x' + (contracts[ind]._source.blockNumber).toString(16);
      var codeQueryString = '{"jsonrpc":"2.0","method":"eth_getCode","id":1,"params":["' + contracts[ind]._source.contractAddress + '","' + block + '"]}';
