...
var elasticClient = {
  ...
  updateContracts: function (data, callback) {
    ...
    for (var i = 0; i < data.length; i++) {
      ids[i] = data[i]._id;
      if (data[i]._source.balance) {
        data[i] = { balance: data[i]._source.balance };
      }
      if (data[i]._source.code) {
        data[i] = { code: data[i]._source.code };
      }
    }
...
function enrichBlockData(block) {
  ...
  // Javascript does not have a problem with large integers, ES also seems to cope with large integers
...
function enrichTransactionData(transaction) {
  ...
  // Javascript does not have a problem with large integers, ES also seems to cope with large integers
  transaction.value = parseInt(transaction.value);  // denominated in Wei
