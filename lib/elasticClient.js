...
var elasticClient = {
  ...
  postBlock: function (data, id, callback) {
    logger.debug('Posting block ' + id + ' (0x' + id.toString(16) + ').');
    createElement(enrichBlockData(data), 'block', id, callback);
  },

  postTransactions: function (data, ids, callback) {
    logger.debug('Posting ' + data.length + ' transactions (starting with id=' + ids[0] + ').');
  ...
  getTransactionsForContracts: function (blockNumber, maxBlockNumber, callback) {
    logger.debug('Getting transactions for contract (blockNumber=' + blockNumber + ').');
    client.search({
  ...
  getContracts: function (blockNumber, maxBlockNumber, callback) {
    logger.debug('Getting contracts from block (blockNumber=' + blockNumber + ').');
    client.search({
  ...
  postContracts: function (data, callback) {
    logger.debug('Posting contracts (first id=' + (data[0]._id || null) + ').');
    var ids = {};
    for (var i = 0; i < data.length; i++) {
      ids[i] = data[i]._id;
  ...
  updateContracts: function (data, callback) {
    logger.debug('Update contracts (first id=' + (data[0]._id || null) + ').');
    var ids = {};
    var update = [];
    for (var i = 0; i < data.length; i++) {
      ids[i] = data[i]._id;
...
function ping() {
  client.ping({
    ...
    if (error) {
      logger.fatal('Elasticsearch cluster is down!');
      process.exit(1);
    } else {
...
function createElement(data, type, id, callback) {
  logger.trace("id:" + id + " type:" + type + " size(body):" + JSON.stringify(data).length);
  client.create({
    ...
    if (error) {
      if (!error.status && error.message === "No Living connections") {
        logger.warn('Retrying \'createElement\'');
        setTimeout(function () {
        ...
        // request entity too large -> increase max post size in Elasticsearch/Nginx reverse proxy
        logger.error(error.message);
        callback(error, response);        
      } else {
        // ToDo: catch the case of "Bad Gateway" here! this keeps reoccuring in our setup
        logger.error(error.message);
        callback(error, response);
      }
    } else {
...
function createBulkElement(data, type, ids, callback) {
  ...
  logger.debug("createBulkElement: size(body)=" + JSON.stringify(data).length);
  ...
  client.bulk({
    ...
    if (error) {
      logger.error(error.message);
      if (!error.status && error.message === "No Living connections") {
        logger.warn('Retrying \'createBulkElement\'');
        setTimeout(function () {
          createBulkElement(data, type, ids, callback);
        }, Number(config.get('elasticsearch.retry_delay')));
      } else if(error.status === 413) {
        // "Request size too large"
        logger.warn("Please increase request size on Elasticsearch/Nginx reverse proxy!");        
      } else {
        logger.fatal('Giving up on error(' + error.status + '): ' + error.message);
        process.exit(1);
      }
    } else {
...
function updateBulkElement(data, type, ids, callback) {
  ...
  client.bulk({
    ...
    if (error) {
      logger.error(error);
      if (!error.status && error.message === "No Living connections") {
        logger.warn('Retrying \'updateBulkElement\'');
        setTimeout(function () {
          updateBulkElement(data, type, ids, callback);
        }, Number(config.get('elasticsearch.retry_delay')));
      } else {
        logger.fatal('Giving up on error(' + error.number + '): ' + error.message);
        process.exit(1);
      }
    } else {
      if (response && response.errors) {
