...
var elasticClient = {
  ...
  updateContracts: function (data, callback) {
    ...
    for (var i = 0; i < data.length; i++) {
      ids[i] = data[i]._id;
      if (data[i]._source && data[i]._source.balance) {
        data[i] = {balance: data[i]._source.balance};
      }
      if (data[i]._source && data[i]._source.code) {
        data[i] = {code: data[i]._source.code, codeLength: data[i]._source.codeLength};
      }
    }
    updateBulkElement(data, 'contract', ids, callback);
  },
...
function enrichBlockData(block) {
  ...
  block.difficulty = parseFloat(parseInt(block.difficulty) / 1000000);
...
function enrichTransactionData(transaction) {
  ...
  transaction.value = parseFloat(parseInt(transaction.value) / 1000000000); // denominated in Gwei
