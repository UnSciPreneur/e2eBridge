  ...
  updateContracts: function (data, callback) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i]._source && data[i]._source.hasOwnProperty('balance')) {
        // ToDo: we should use our standard quotient here, right?
...
function setupIndexAndMappings() {
  client.indices.create({
    index: config.get('elasticsearch.indices.blocks.name'),
    body: {
      "number_of_shards": config.get('elasticsearch.indices.blocks.number_of_shards'),
      "number_of_replicas": config.get('elasticsearch.indices.blocks.number_of_replicas')
    }
  }).then(function (resp) {
    logger.trace(resp);
    logger.info('Index [' + config.get('elasticsearch.indices.blocks.name') + '] created successfully.');
  ...
  client.indices.create({
    index: config.get('elasticsearch.indices.transactions.name'),
    body: {
      "number_of_shards": config.get('elasticsearch.indices.transactions.number_of_shards'),
      "number_of_replicas": config.get('elasticsearch.indices.transactions.number_of_replicas')
    }
  }).then(function (resp) {
    logger.trace(resp);
    logger.info('Index [' + config.get('elasticsearch.indices.transactions.name') + '] created successfully.');
  ...
  client.indices.create({
    index: config.get('elasticsearch.indices.contracts.name'),
    body: {
      "number_of_shards": config.get('elasticsearch.indices.contracts.number_of_shards'),
      "number_of_replicas": config.get('elasticsearch.indices.contracts.number_of_replicas')
    }
  }).then(function (resp) {
    logger.trace(resp);
    logger.info('Index [' + config.get('elasticsearch.indices.contracts.name') + '] created successfully.');

    var contractMapping = require('../config/ethereum/contractMapping.json');
    client.indices.putMapping({
      index: config.get('elasticsearch.indices.contracts.name'),
      body: contractMapping
    }).then(function (resp) {
      logger.trace(resp);
      logger.info('Mappings for type contract created successfully.');
    }, function (err) {
      logger.error(err.message);
    });
  }, function (err) {
...
function deleteIndex(callback) {
  client.indices.get({index: config.get('elasticsearch.indices.blocks.name')}).then(function (resp) {
    logger.trace(resp);
    client.indices.delete({
      index: config.get('elasticsearch.indices.blocks.name')
    }).then(function (resp) {
      logger.trace(resp);
      logger.warn('Index [' + config.get('elasticsearch.indices.blocks.name') + '] deleted');
      callback();
    }, function (err) {
  ...
  client.indices.get({index: config.get('elasticsearch.indices.transactions.name')}).then(function (resp) {
    logger.trace(resp);
    client.indices.delete({
      index: config.get('elasticsearch.indices.transactions.name')
    }).then(function (resp) {
      logger.trace(resp);
      logger.warn('Index [' + config.get('elasticsearch.indices.transactions.name') + '] deleted');
      callback();
    }, function (err) {
      logger.error('Unexpected error during index deletion: ' + err.message);
    });
  ...
  client.indices.get({index: config.get('elasticsearch.indices.contracts.name')}).then(function (resp) {
    logger.trace(resp);
    client.indices.delete({
      index: config.get('elasticsearch.indices.contracts.name')
    }).then(function (resp) {
      logger.trace(resp);
      logger.warn('Index [' + config.get('elasticsearch.indices.contracts.name') + '] deleted');
      callback();
    }, function (err) {
      logger.error('Unexpected error during index deletion: ' + err.message);
    });
...
function enrichContractData(contract) {
  ...
  contract.inputHash = sha3.digest('hex').substr(0, 15);
